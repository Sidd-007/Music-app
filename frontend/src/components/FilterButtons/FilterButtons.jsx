...
const FilterButtons = ({ filterData, flag }) => {
    ...
    return (
        <div className="border border-white rounded-md px-4 py-1 relative cursor-pointer hover:border-[#00BD95]">
            <p
                className="text-base tracking-wide text-gray-100 flex items-center gap-2 "
            ...
            >
                ...
                <IoChevronDown
                    className={`text-base text-[#00BD95] duration-150 transition-all ease-in-out ${filterMenu ? "rotate-180" : "rotate-0"
                        }`}
                />
            </p>
            {filterData && filterMenu && (
                <motion.div
                    ...
                    exit={{ opacity: 0, y: 50 }}
                    className="w-48 z-50 bg-[#1E1E1E] max-h-44 overflow-y-scroll scrollbar-thin py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0"
                >
                    {filterData?.map((data) => (
                        <div
                            key={data.name}
                        ...
                        >
                            {(flag === "Artist" || flag === "Albums") && (
                                <img
                                    src={data.imageURL}
                                    className="w-8 min-w-[32px] h-8 rounded-full border-[#00BD95] object-cover"
                                    alt=""
                                />
                            )}
